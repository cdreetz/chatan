...
class LiveViewer:
    ...
    def _chatan_to_ui_columns(self, schema: Dict[str, Any], metadata: Dict[str, Any]) -> List[Dict]:
        ...
        for col in ui_columns:
            ...
            if col_type == "prompt":
                ...
                if self.generator_client:
                    ...
                    schema[name] = lambda ctx, prompt=config.get("prompt", ""): f"Mock: {prompt[:20]}..."
                ...
                if self.generator_client:
                    ...
                    # Mock reference
                    schema[name] = lambda ctx, tmpl=template: f"Mock ref: {tmpl[:20]}..."
        
        return schema
    
    def set_generator_client(self, provider: str, api_key: str):
        """Set the generator client for LLM generation."""
        try:
            self.generator_client = generator(provider, api_key)
...
<body>
    ...
    async def _delete_column(self, request):
        ...
        try:
            ...
            self.ui_columns = [col for col in self.ui_columns if col['name'] != column_name]
    ...
    async def _start_generation(self, request):
        ...
        if self.is_generating:
            return web.json_response({"error": "Generation already in progress"}, status=400)
        ...
        try:
            ...
            if not chatan_schema:
                return web.json_response({"error": "No valid columns to generate"}, status=400)
    ...
    async def _serve_data(self, request):
        ...
        try:
            ...
            return web.Response(text='{"rows": [], "completed": false, "current_rows": {}}', 
    ...
    async def _export_data(self, request):
        ...
        try:
            ...
            if format_type == 'csv':
                ...
                return web.Response(text=csv_content, 
                                  content_type='text/csv',
                                  headers={'Content-Disposition': 'attachment; filename="dataset.csv"'})
            
            elif format_type == 'json':
                return web.Response(text=json.dumps(rows, indent=2),
                                  content_type='application/json',
...
async def launch_viewer(title: str = "Chatan Dataset Builder", 
                       provider: str = None, 
                       api_key: str = None,
                       auto_open: bool = True) -> LiveViewer:
